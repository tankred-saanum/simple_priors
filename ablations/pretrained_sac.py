...
parser.add_argument('--num_episodes', metavar='N', type=int, default = 1000,
                    help='number of episodes in experiment')
...
parser.add_argument('--model_name', metavar='N', type=str, default = "sac_pretrained",
                    help='suite domain name')
...
def train_sac_dmc(seed, args):
    ...
    eval_counter = 0
    with open(f'ablations/pretrained_transformer_models_v2.pkl', 'rb') as f:
        transformer_dict = pickle.load(f)
        #agents, all_scores, args = CPU_Unpickler(f).load()#pickle.load(f)#CPU_Unpickler(f).load()#CPU_Unpickler(f).load()##CPU_Unpickler(f).load()

    pretrained_transformer = transformer_dict[args.dom_name + args.task_name]
    ...
    agent = SPAC(state_dims=num_features, action_dims = action_dims, hidden_dims=args.hidden_dims, chunk_length=args.chunk_length, gamma=0.99, alpha=args.alpha,
                    rho=args.rho, embedding_dim=None, nlayers=None, nheads=None, pretrained_sequence_model = pretrained_transformer)
...
with open(f'results/{args.model_name}/{args.dom_name}_{args.task_name}_alpha={args.alpha}_chunk_lenth={args.chunk_length}_ar={args.action_repeat}_nseed={args.num_seeds}_nep{args.num_episodes}_nh={args.hidden_dims}_lmbd={args.lmbd}.pkl', 'wb') as file:
