...
import models.sac
reload(models.sac)
from models.sac import SACContinuous, LZSAC, ContinuousRPC, SPAC#, Critic, lzPO, StatelzPO
import models.utils
reload(models.utils)
from models.utils import *
...
parser.add_argument('--num_episodes', metavar='N', type=int, default = 200,
                    help='number of episodes in experiment')
...
parser.add_argument('--batch_size', metavar='N', type=int, default = 128,
                    help='length of sequence used to condition sequence model for action prediction')
parser.add_argument('--chunk_length', metavar='N', type=int, default = 20,
                    help='length of sequence used to condition sequence model for action prediction')
...
parser.add_argument('--dom_name', metavar='N', type=str, default = "cheetah",
                    help='suite domain name')
parser.add_argument('--task_name', metavar='N', type=str, default = "run",
                    help='suite domain name')
...
def train_sac_dmc(seed, args, n_heads, n_emb, n_layers):
    ...
    agent = SPAC(state_dims=num_features, action_dims = action_dims, hidden_dims=args.hidden_dims, chunk_length=args.chunk_length, gamma=0.99, alpha=args.alpha, rho=args.rho, embedding_dim=n_emb, nlayers=n_layers, nheads=n_heads)

    optimizer_actor = optim.Adam(list(agent.policy_head.parameters()),lr=args.lr)
    #optimizer_actor = optim.Adam([{'params':agent.policy_head.encoder.parameters()}, {'params' : agent.sequence_prior.parameters(), 'lr': 0.0003}],lr=args.lr)
    optimizer_sequence_model = optim.Adam(list(agent.sequence_prior.parameters()),lr=0.0003)
...
all_scores = torch.zeros(len(nlayer), args.num_seeds, int(args.num_episodes/args.eval_freq)+1, args.num_test_episodes)

for i, (n_heads, n_emb, n_layers) in enumerate(zip(heads, emb, nlayer)):
    for seed in range(args.num_seeds):
        agent, scores = train_sac_dmc(seed, args, n_heads=n_heads, n_emb=n_emb, n_layers = n_layers)
        agents.append(agent)
...
# plt.plot(all_scores_new.mean(dim=1).mean(dim=-1).T)


with open(f'results/transformer_size_results/{args.dom_name}_{args.task_name}_alpha={args.alpha}_chunk_lenth={args.chunk_length}_ar={args.action_repeat}_nseed={args.num_seeds}_nep{args.num_episodes}_nh={args.hidden_dims}_lmbd={args.lmbd}.pkl', 'wb') as file:
